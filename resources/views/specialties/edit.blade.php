<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Specialty: ') }} {{ $specialty->name }}</h2>
    </x-slot>
                ...
                <form method="POST" action="{{ route('specialties.update', $specialty) }}">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $specialty->name)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('name')" />
                        </div>

                        <div>
                            <x-input-label for="description" :value="__('Description')" />
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $specialty->description) }}</textarea>
                        ...
                        <div class="flex items-center justify-end gap-4">
                            ...
                            <x-primary-button>{{ __('Update Specialty') }}</x-primary-button>
                        </div>
                    </div>
                </form>
            </div>
